...
# Complete infrastructure: VPC + ALB + ECS Fargate + RDS
# =============================================================================
...
locals {
  ...
  database_port = var.database_engine == "postgres" ? 5432 : 3306
}
...
resource "aws_subnet" "database" {
  ...
  vpc_id            = aws_vpc.main.id
  cidr_block        = cidrsubnet(var.vpc_cidr, 4, count.index + 2 * length(local.availability_zones))
  availability_zone = local.availability_zones[count.index]
...
resource "aws_secretsmanager_secret_version" "database" {
  ...
  secret_string = jsonencode({
    ...
    url      = "${var.database_engine}://${var.database_username}:${random_password.database.result}@${aws_db_instance.main.address}:${local.database_port}/${var.database_name}"
  })
}
